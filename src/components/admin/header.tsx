...
import { Menu, Search, Bell, ChevronDown, Globe, Sun, Moon, PanelLeftClose, PanelLeft } from 'lucide-react';
...
export function Header() {
  const { currentView, sidebarOpen, toggleSidebar, theme, toggleTheme } = useApp();
    ...
    <header className="h-16 bg-white dark:bg-gray-900 border-b border-gray-200 dark:border-gray-700 flex items-center justify-between px-4 lg:px-6 shrink-0 transition-colors duration-300">
      <div className="flex items-center gap-3">
        ...
        <div className="hidden lg:flex items-center gap-2 text-sm">
          <span className="text-gray-400 dark:text-gray-500">Admin Panel</span>
      ...
      <div className="flex items-center gap-1.5">
        ...
        <div className="flex items-center gap-2 ml-1">
          <div className="w-8 h-8 rounded-full bg-[#6366F1] flex items-center justify-center text-white text-xs font-semibold transition-transform duration-200 hover:scale-110">
            SA
          </div>
          <div className="hidden sm:block">
            <p className="text-sm font-medium text-gray-800 dark:text-gray-100 leading-tight">Super Admin</p>
            <p className="text-[11px] text-gray-400 dark:text-gray-500">admin@admin.com</p>
          </div>
        </div>
      </div>
    </header>
