...
export function SpamWordsView() {
  ...
  const fetchData = useCallback(async () => {
    ...
    setError(null);
    try {
      const res = await fetch('/api/spam-words');
      const json = await res.json();
      if (json.data) {
        setWords(json.data.map((w: SpamWord) => ({ ...w, word: w.word.toUpperCase() })));
      } else {
        setWords([]);
      }
    } catch {
      setError('Failed to load spam words');
    } finally {
      setLoading(false);
    }
  }, []);
  ...
  if (loading) {
    return (
      <div className="flex flex-col items-center justify-center py-16 gap-3">
        ...
        <p className="text-sm text-gray-500">Loading spam words...</p>
      </div>
    );
  }
