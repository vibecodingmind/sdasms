...
interface Currency {
  id: number; name: string; code: string; symbol: string; rate: number; status: string;
}
...
export function CurrenciesView() {
  ...
  useEffect(() => {
    fetch('/api/currencies').then((r) => r.json()).then((r) => setCurrencies(r.data || [])).catch(() => {});
  }, []);
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-gray-500 mt-0.5">Manage supported currencies</p>
        </div>
        <Dialog open={dialogOpen} onOpenChange={setDialogOpen}>
          <DialogTrigger asChild>
            <Button className="bg-[#6366F1] hover:bg-[#5558E6] text-white"><Plus className="h-4 w-4 mr-2" /> Add Currency</Button>
          </DialogTrigger>
          <DialogContent>
            ...
            <form className="space-y-4" onSubmit={(e) => { e.preventDefault(); setDialogOpen(false); }}>
              <div><label className="block text-sm font-medium text-gray-700 mb-1">Currency Name</label><Input placeholder="US Dollar" /></div>
              <div className="grid grid-cols-3 gap-4">
                <div><label className="block text-sm font-medium text-gray-700 mb-1">Code</label><Input placeholder="USD" /></div>
                <div><label className="block text-sm font-medium text-gray-700 mb-1">Symbol</label><Input placeholder="$" /></div>
                <div><label className="block text-sm font-medium text-gray-700 mb-1">Rate</label><Input type="number" step="0.01" placeholder="1.00" /></div>
              </div>
          ...
          <Table>
            <TableHeader>
              <TableRow className="bg-gray-50/50">
                <TableHead className="text-xs text-gray-500 font-medium">Currency Name</TableHead>
                <TableHead className="text-xs text-gray-500 font-medium">Code</TableHead>
                <TableHead className="text-xs text-gray-500 font-medium">Symbol</TableHead>
                <TableHead className="text-xs text-gray-500 font-medium">Exchange Rate</TableHead>
            ...
            <TableBody>
              {currencies.map((c) => (
                <TableRow key={c.id} className="hover:bg-gray-50/50">
                  ...
                  <TableCell className="text-sm text-gray-500">{c.symbol}</TableCell>
                  <TableCell className="text-sm text-gray-600">{c.rate}</TableCell>
                  <TableCell>
