...
type SortField = 'sender_id' | 'status';
  ...
  return (
    <ArrowUpDown
      className={`ml-1 h-3.5 w-3.5 inline ${
        sortField === field ? 'text-[#D72444]' : 'text-gray-400 dark:text-gray-500'
      }`}
    />
  );
}
...
export function SenderIdsView() {
  ...
  const fetchSenderIds = useCallback(async () => {
    try {
      ...
      if (json.success) {
        ...
        toast.error(json.message || 'Failed to fetch sender IDs');
      }
    } catch {
  ...
  const handleSave = async () => {
    ...
    try {
      const res = await fetch('/api/sender-ids', {
        ...
        body: JSON.stringify({ sender_id: formSenderId }),
      });
      const json = await res.json();
      if (json.success) {
        ...
        toast.error(json.message || 'Failed to create sender ID');
      }
    } catch {
  ...
  const handleDelete = async () => {
    if (deletingId !== null) {
      try {
        const res = await fetch(`/api/sender-ids?id=${deletingId}`, {
          method: 'DELETE',
        });
        const json = await res.json();
        if (json.success) {
          ...
          toast.error(json.message || 'Failed to delete sender ID');
        }
      } catch {
        ...
        <CardContent className="p-0">
          ...
          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 px-4 py-3 border-b border-gray-100 dark:border-gray-800">
            <div className="flex items-center gap-2">
              ...
              >
                <SelectTrigger className="w-[70px] h-8 text-xs">{entriesPerPage}</SelectTrigger>
                <SelectContent>
              ...
              <span className="text-xs text-gray-500 dark:text-gray-400 whitespace-nowrap">entries</span>
            </div>
          ...
          <div className="overflow-x-auto">
            <table className="w-full">
              ...
              <tbody>
                {paged.length === 0 && (
                  <tr>
                    <td colSpan={5} className="text-center py-12 text-sm text-gray-400 dark:text-gray-500">
                  ...
                  return (
                    ...
                    >
                      ...
                      <td className="px-4 py-3">
                        <span className="text-sm font-bold text-gray-800 dark:text-gray-100">{item.sender_id}</span>
                      </td>
                      <td className="px-4 py-3">
                        <span
                      ...
                      <td className="px-4 py-3">
                        <span className="text-xs text-gray-400 dark:text-gray-500">
                          {item.created_at ? new Date(item.created_at).toLocaleDateString('en-US', { day: 'numeric', month: 'short', year: '2-digit' }) : '—'}
                        </span>
                      </td>
                      <td className="px-4 py-3">
